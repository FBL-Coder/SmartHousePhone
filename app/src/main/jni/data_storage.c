...
void set_rcuinfo(UDPPROPKT *pkt, SOCKADDR_IN sender) {
    ...
    gw_client_add(&gw_client_list, sender, pkt->uidSrc, (u8 *) "", rcuinfo->IpAddr,
                  gw_client_list.size);
    ...
    switch (pkt->subType2) {
        case e_ware_airCond:
            ...
            for (int i = 0; i < dev_cnt; i++) {
                ...
                ware_aircond_add(&aircond_list, *ware_dev, *aircond, pkt->uidSrc,
                                 aircond_list.size);
        ...
        case e_ware_curtain:
            ...
            for (int i = 0; i < dev_cnt; i++) {
                ...
                ware_curtain_add(&curtain_list, *ware_dev, *curtain, pkt->uidSrc,
                                 curtain_list.size);
            }
            break;
...
void set_key_opitem(UDPPROPKT *pkt) {
    ...
    for (int i = 0; i < cnt; i++) {
        ...
        keyop_item_add(&keyop_item_list, *keyop_item, pkt->uidSrc, keyinput_board_id, pkt->subType2,
...
void del_key_opitem(UDPPROPKT *pkt) {
    ...
    keyop_item_remove(&keyop_item_list, pkt->uidSrc, keyinput_board_id, pkt->subType2);
}
...
void set_chn_opitem(UDPPROPKT *pkt) {
    ...
    chnop_item_clean(&chnop_item_list);
    for (int i = 0; i < cnt; i++) {
        CHNOP_ITEM *chnop_item = (CHNOP_ITEM *) (pkt->dat + 14 + sizeof(CHNOP_ITEM) * i);

        chnop_item_add(&chnop_item_list, *chnop_item, pkt->uidSrc, chn_board_id, devType, devID,
