...
UDPPROPKT *udp_pkt_bradcast(u8 *devUnitID, u8 *rcu_pwd,char *dst_ip) {
    ...
    unsigned long sender_ip = inet_addr(dst_ip);
    unsigned char *dstip = (unsigned char *) &sender_ip;
...
void handShake(UDPPROPKT *pkt) {
    UDPPROPKT *send_pkt = pre_send_udp_pkt(inet_addr(inet_ntoa(sender.sin_addr)), 0, 0,
                                           e_udpPro_handShake, pkt->uidSrc, pkt->pwdDst, NOW_ACK, 0,
                                           0);
