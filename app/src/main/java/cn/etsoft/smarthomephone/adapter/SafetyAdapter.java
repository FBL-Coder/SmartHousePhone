...
public class SafetyAdapter extends BaseAdapter {
    ...
    public SafetyAdapter(Context context,SetSafetyResult result) {
    ...
    public void notifyDataSetChanged(SetSafetyResult result) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (list.size() == 0) {
            ToastUtil.showToast(context, "没有收到防区信息");
            return convertView;
        }
        if (list.size() > 0) {
            viewHolder.image.setImageResource(image[position % 5]);
            viewHolder.title.setText(list.get(position).getSecName());
