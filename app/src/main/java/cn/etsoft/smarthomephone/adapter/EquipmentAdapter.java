...
import cn.etsoft.smarthomephone.domain.EquipmentBean;
...
public class EquipmentAdapter extends BaseAdapter {
    ...
    public EquipmentAdapter(List<String> title, int[] image, Context context) {
        super();
        listViewItems = new ArrayList<EquipmentBean>();
        mInflater = LayoutInflater.from(context);
        for (int i = 0; i < title.size(); i++) {
            EquipmentBean item = new EquipmentBean(title.get(i), image[i]);
