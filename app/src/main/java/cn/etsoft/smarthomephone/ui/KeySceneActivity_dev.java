...
import cn.etsoft.smarthomephone.adapter.KeyAdapter_keyScene;
...
public class KeySceneActivity_dev extends Activity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        if (!IsHaveData) {
            ToastUtil.showToast(this, "获取数据异常，请稍后在试");
            return;
        }
        switch (view.getId()) {
            ...
            case R.id.title_bar_tv_room:
                if (MyApplication.mInstance.getKey_scene_data() == null) {
                    ToastUtil.showToast(this, "没有按键信息，不能保存");
                    return;
                }
                ...
                builder.setPositiveButton("保存", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (chnOpItem_scene.getKey2scene_item().size() > 12) {
                            ToastUtil.showToast(KeySceneActivity_dev.this, "最多只能添加12个按键");
                            return;
                        } else {
