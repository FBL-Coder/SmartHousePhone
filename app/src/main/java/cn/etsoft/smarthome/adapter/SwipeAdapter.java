    ...
    public View getView(final int position, View contentView, ViewGroup arg2) {
        ...
        if (keyOpItems.get(position).getDevType() == 0) {
            ...
            for (int i = 0; i < list.size(); i++) {
                if (list.get(i).getDev().getDevId() == keyOpItems.get(position).getDevId() && list.get(i).getDev().getCanCpuId().equals(keyOpItems.get(position).getOut_cpuCanID())) {
        ...
        } else if (keyOpItems.get(position).getDevType() == 7) {
            List<WareFreshAir> freshAirs = MyApplication.getWareData().getFreshAirs();
            for (int i = 0; i < freshAirs.size(); i++) {
                if (freshAirs.get(i).getDev().getDevId() == keyOpItems.get(position).getDevId()
                        && freshAirs.get(i).getDev().getCanCpuId().equals(keyOpItems.get(position).getOut_cpuCanID())) {
                    viewHolder.title.setText(freshAirs.get(i).getDev().getDevName() + "");
                }
            }
            viewHolder.deploy_iv.setImageResource(image[4]);
            text = new String[]{"打开", "高风", "中风", "低风", "关闭"};
            viewHolder.choose.setText(text[keyOpItems.get(position).getKeyOpCmd()]);
            if (keyOpItems.get(position).getKeyOp() == 1) {
                viewHolder.choose1.setText("弹起");
            } else if (keyOpItems.get(position).getKeyOp() == 0) {
        ...
        } else if (keyOpItems.get(position).getDevType() == 9) {
            List<WareFloorHeat> floorHeats = MyApplication.getWareData().getFloorHeat();
            for (int i = 0; i < floorHeats.size(); i++) {
                if (floorHeats.get(i).getDev().getDevId() == keyOpItems.get(position).getDevId()
                        && floorHeats.get(i).getDev().getCanCpuId().equals(keyOpItems.get(position).getOut_cpuCanID())) {
                    viewHolder.title.setText(floorHeats.get(i).getDev().getDevName() + "");
                }
            }
            viewHolder.deploy_iv.setImageResource(image[4]);
            text = new String[]{"未设置", "打开", "关闭"};
            viewHolder.choose.setText(text[keyOpItems.get(position).getKeyOpCmd()]);
            if (keyOpItems.get(position).getKeyOp() == 1) {
                viewHolder.choose1.setText("弹起");
            } else if (keyOpItems.get(position).getKeyOp() == 0) {
